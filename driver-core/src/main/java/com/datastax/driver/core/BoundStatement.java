...
public class BoundStatement extends Statement implements GettableData {
    ...
    public <T> BoundStatement setList(int i, List<T> v) {
        ...
        DataType cqlType = metadata().getType(i);
        @SuppressWarnings("unchecked")
        TypeToken<T> eltType = (TypeToken<T>)  getCodecRegistry().codecFor(cqlType.getTypeArguments().get(0)).getJavaType();
        return setValue(i, wrapper.codecFor(i, listOf(eltType)).serialize(v));
    }
    ...
    public <K, V> BoundStatement setMap(int i, Map<K, V> v) {
        ...
        DataType cqlType = metadata().getType(i);
        @SuppressWarnings("unchecked")
        TypeToken<K> keysType = (TypeToken<K>)  getCodecRegistry().codecFor(cqlType.getTypeArguments().get(0)).getJavaType();
        @SuppressWarnings("unchecked")
        TypeToken<V> valuesType = (TypeToken<V>)  getCodecRegistry().codecFor(cqlType.getTypeArguments().get(1)).getJavaType();
        return setValue(i, wrapper.codecFor(i, mapOf(keysType, valuesType)).serialize(v));
    }
    ...
    public <T> BoundStatement setSet(int i, Set<T> v) {
        ...
        @SuppressWarnings("unchecked")
        TypeToken<T> eltType = (TypeToken<T>)  getCodecRegistry().codecFor(cqlType.getTypeArguments().get(0)).getJavaType();
        return setValue(i, wrapper.codecFor(i, setOf(eltType)).serialize(v));
    }
